...
class RecordCreate(BaseModel):
    ...
    def validate_timestep(cls, value: str) -> str:
        """Validate and normalize the timestep field to YYYY-MM-DD HH:MM format."""
        try:
...
async def validation_exception_handler(_request: Request, exc: RequestValidationError) -> JSONResponse:
    """Return a structured JSON error body for validation failures."""
...
async def unhandled_exception_handler(_request: Request, exc: Exception) -> JSONResponse:
    """Return a structured JSON error body for unhandled exceptions."""
    return JSONResponse(
...
def ensure_data_file() -> None:
    """Raise FileNotFoundError if the CSV is missing; add an id column if absent."""
...
def load_df() -> pd.DataFrame:
    """Load and validate the CSV, returning only the expected columns."""
    ensure_data_file()
    df = pd.read_csv(DATA_FILE)
...
def save_df(df: pd.DataFrame) -> None:
    """Persist the DataFrame to the CSV file."""
    df.to_csv(DATA_FILE, index=False)
...
def get_records() -> list[dict[str, Any]]:
    """Return all records from the CSV as a list of dicts."""
    df = load_df()
...
def create_record(record: RecordCreate) -> dict[str, Any]:
    """Append a new record to the CSV and return it with its assigned id."""
    with CSV_LOCK:
        df = load_df()
...
def delete_record(record_id: int) -> dict[str, str]:
    """Delete the record with the given id; raise 404 if not found."""
    with CSV_LOCK:
        df = load_df()
